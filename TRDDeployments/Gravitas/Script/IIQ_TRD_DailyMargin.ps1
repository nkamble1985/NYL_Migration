...
try {
    ...
    $emailId      = $graphSecret.email_Id  # Sender email
...
Try {
    ...
    $filedatetime = Get-LastBusinessDay -Date (Get-Date) -DateFormat "MMddyyyy"

    $ExecutablePath = "C:\Deployments\ExcelMergerProd\ExcelMerger\ExcelMerger.exe"
    & $ExecutablePath $runDate

    LogWrite "Daily Margin Execution End  --> for run date $runDate"

    # ==============================
    # Check if Daily Margin file exists
    # ==============================
    $filePath = $outputFolder + $fileNamePrefix + $filedatetime + ".xlsx"
    LogWrite "Checking output file path --> $filePath"

    if (Test-Path $filePath -PathType Leaf) {
        LogWrite "Daily Margin file generated at $filePath."
        Send-ToEmail -recipientEmail $toEmail `
                     -emailMessage "Daily Margin executed successfully and file generated at $filePath." `
                     -emailSubject "$jobname - File Generated Successfully"
    } else {
        $failMsg = "Daily Margin execution completed but file not found at $filePath"
        LogWrite $failMsg
        Send-ToEmail -recipientEmail $toEmail `
