...
# Config
# ==============================
$Logfile        = "C:\Gravitas\Logs\Execute_Variation_Margin_SODLog.txt"
$SummaryLogfile = "C:\Gravitas\Logs\EquitizerSummaryLog.txt"
$jobname        = "Execute_Variation_Margin_SOD"

$errorEmail     = "nitin_kamble@nylim.com"
$region         = "us-east-1"  # AWS region for secrets
$toEmail        = "nitin_kamble@nylim.com"  # Recipient email

$outputFolder   = "C:\Gravitas\SharedFiles\VariationMargin\"  # Output folder
$filePrefix     = "Variation_Margin_"
...
Try {
    ...
    $ExecutablePath = "C:\Deployments\VMMergerProd\VMMerger\VMMerger.exe"
    & $ExecutablePath $runDate

    LogWrite "Variation Margin SOD Execution End  --> for run date $runDate"
    ...
    if (Test-Path $filePath -PathType Leaf) {
        LogWrite "Variation Margin SOD file generated at $filePath."
        Send-ToEmail -recipientEmail $toEmail `
                     -emailMessage "Variation Margin SOD executed successfully and file generated at $filePath." `
                     -emailSubject "$jobname - File Generated Successfully"
    } else {
        $failMsg = "Variation Margin SOD execution completed but file not found at $filePath"
        LogWrite $failMsg
        Send-ToEmail -recipientEmail $toEmail `
