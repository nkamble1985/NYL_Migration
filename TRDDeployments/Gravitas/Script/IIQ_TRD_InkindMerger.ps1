...
$outputFolder   = "C:\Gravitas\SharedFiles\InKind\"  # Output folder
$filePrefix     = "Enhanced_Basket_AutoLoad_"
...
Try {
    ...
    if (Test-Path $filePath -PathType Leaf) {
        LogWrite "InKind SOD file generated at $filePath."
        Send-ToEmail -recipientEmail $toEmail `
                     -emailMessage "InKind SOD executed successfully and file generated at $filePath." `
                     -emailSubject "$jobname - File Generated Successfully"
    } else {
        $failMsg = "InKind SOD execution completed but file not found at $filePath"
        LogWrite $failMsg
        Send-ToEmail -recipientEmail $toEmail `
